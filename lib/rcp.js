...
const fetch_last_proposal_id = async (chain) => {
  const url = `http:///a-${chain}--${process.env.NOTIONAL_API_KEY}.gw.notionalapi.net/cosmos/gov/v1/proposals?pagination.limit=1&pagination.count_total=false&pagination.reverse=true`;
  // console.log(url);
  const response = await fetch(url);
  if (response.status === 200) {
    const data = await response.json();
